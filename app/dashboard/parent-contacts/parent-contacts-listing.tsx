...
async function ParentContactsListingTable({
  requestedPage,
  filters,
  listBasePath,
}: {
  ...
  const { contacts, totalCount, page, pageSize } = await getParentContacts(
    requestedPage,
    filters,
    listBasePath
  )
  ...
  const prevHref =
    page > 1 ? `${listBasePath}${buildParentContactsQueryString({ ...base, page: page - 1 })}` : null
  const totalPages = Math.max(1, Math.ceil(totalCount / pageSize))
  const nextHref =
    page < totalPages ? `${listBasePath}${buildParentContactsQueryString({ ...base, page: page + 1 })}` : null
...
type ParentContactsListPageContentProps = {
...
export async function ParentContactsListPageContent({
  searchParams,
  listBasePath,
  headingTitle,
  headingDescription = 'Manage emergency contacts and guardians for passengers',
  cardTitle = 'All parent contacts',
}: ParentContactsListPageContentProps) {
  ...
  const suspenseKey = JSON.stringify({ base: listBasePath, ...filters, page: requestedPage })

  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-navy">{headingTitle}</h1>
          <p className="mt-2 text-sm text-gray-600">{headingDescription}</p>
        </div>
      ...
      <Suspense fallback={<div className="h-24 w-full max-w-3xl rounded-lg bg-gray-100 animate-pulse" />}>
        <ParentContactsSearchFilters basePath={listBasePath} />
      </Suspense>
