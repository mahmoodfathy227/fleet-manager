...
export default function RouteDocumentsCard({ routeId }: { routeId: number }) {
  const supabase = createClient()
  const [documents, setDocuments] = useState<RouteDocumentRow[]>([])
  ...
  const loadDocuments = async () => {
    ...
    if (error) {
      console.error('Error fetching route documents', error)
      setDocuments([])
    } else {
      setDocuments((data as RouteDocumentRow[]) || [])
    }
    setLoading(false)
  }
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const storagePath = `routes/${routeId}/${ts}_${rand}.${ext}`
  ...
  const removeStorageObject = async (path: string | null) => {
    ...
    if (rmErr) console.warn('[RouteDocumentsCard] storage remove', path, rmErr)
  }

  const handleDeleteDocument = async (doc: RouteDocumentRow) => {
    const label = doc.title || doc.file_name || 'this document'
    if (!window.confirm(`Delete “${label}”? This cannot be undone.`)) return
    setError(null)
    setDeletingDocId(doc.id)
    console.debug('[RouteDocumentsCard] delete start', { documentId: doc.id, routeId })
    try {
      ...
      console.debug('[RouteDocumentsCard] delete done', { documentId: doc.id })
      await loadDocuments()
    } catch (err: any) {
      console.error('[RouteDocumentsCard] delete failed', err)
  ...
  const resolveUrl = (doc: RouteDocumentRow) => {
      ...
      <CardHeader className="bg-navy text-white">
        <div className="flex items-center justify-between">
          <CardTitle className="flex items-center space-x-2">
            ...
            <span>Route Documents ({documents.length})</span>
          </CardTitle>
          ...
          <div className="space-y-3 border rounded-lg p-4">
            <div className="grid gap-3 md:grid-cols-2">
              <div className="space-y-1">
                <Label htmlFor="route-doc-title">Title *</Label>
                <Input
                  id="route-doc-title"
                  value={title}
                  onChange={(e) => setTitle(e.target.value)}
                  placeholder="e.g., Risk Assessment, Contract"
                />
              </div>
              <div className="space-y-1">
                <Label htmlFor="route-doc-file">File *</Label>
                <Input
                  id="route-doc-file"
                  type="file"
            ...
            <div className="space-y-1">
              <Label htmlFor="route-doc-notes">Notes (optional)</Label>
              <textarea
                id="route-doc-notes"
                value={notes}
