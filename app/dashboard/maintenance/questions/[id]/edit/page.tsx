...
export default function EditMaintenanceQuestionPage() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      setForm({
        question_title: (r.question_title as string) ?? '',
        question_desc: (r.question_desc as string) ?? '',
        category: (r.category as string) ?? '',
        severity: (r.severity as MaintenanceSeverity) ?? 'medium',
        image_bool: (r.image_bool as boolean) ?? false,
        video_bool: (r.video_bool as boolean) ?? false,
        icon: (r.icon as string) ?? '',
        icon_path: (r.icon_path as string) ?? '',
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault()
    if (!isValid || saving || !canUpdate) return
    setError(null)
    setSaving(true)
  ...
  return (
    <div className="space-y-6">
      ...
      <h1 className="text-2xl font-bold text-slate-900">Edit maintenance question</h1>
        ...
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="flex gap-2">
              ...
              <Link href={`/dashboard/maintenance/questions/${id}`}>
