...
export default function SchoolDocumentsCard({ schoolId }: { schoolId: number }) {
  ...
  const removeStorageObject = async (path: string | null) => {
    ...
    if (rmErr) console.warn('[SchoolDocumentsCard] storage remove', path, rmErr)
  }

  const handleDeleteDocument = async (doc: SchoolDocumentRow) => {
    const label = doc.title || doc.file_name || 'this document'
    if (!window.confirm(`Delete “${label}”? This cannot be undone.`)) return
    setError(null)
    setDeletingDocId(doc.id)
    console.debug('[SchoolDocumentsCard] delete start', { documentId: doc.id, schoolId })
    try {
      ...
      console.debug('[SchoolDocumentsCard] delete done', { documentId: doc.id })
      await loadDocuments()
    } catch (err: any) {
      console.error('[SchoolDocumentsCard] delete failed', err)
