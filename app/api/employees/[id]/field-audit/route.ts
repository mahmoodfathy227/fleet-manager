...
export async function GET(
  ...
  try {
    ...
    // Get field audit history for this employee
    ...
    if (data) {
      data.forEach((log: any) => {
        ...
        if (!fieldHistory[fieldName] || new Date(log.change_time) > new Date(fieldHistory[fieldName].change_time)) {
          // Handle null user values gracefully
          const userEmail = log.users?.email || null
          const userName = log.users?.employees?.full_name || log.users?.email || 'System'
