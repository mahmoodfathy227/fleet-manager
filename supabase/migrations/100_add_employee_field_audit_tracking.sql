-- ====================================================
-- Field-Level Audit Tracking for Employees
-- Tracks individual field changes for employees
-- ====================================================

-- Ensure field_audit_log table exists (it should from vehicle/route audit migrations)
-- If not, create it
...
-- Function to log field changes for employees
-- Handles null user values gracefully
CREATE OR REPLACE FUNCTION log_employee_field_changes()
RETURNS TRIGGER AS $$
DECLARE
  ...
  IF TG_OP = 'INSERT' THEN
    ...
    LOOP
      ...
      IF new_val IS NOT NULL THEN
        ...
        VALUES ('employees', NEW.id, field_name, NULL, new_val::TEXT, current_user_id, 'CREATE');
      END IF;
    END LOOP;
    RETURN NEW;
  END IF;
  ...
  IF TG_OP = 'UPDATE' THEN
    ...
    LOOP
      ...
      IF (old_val IS DISTINCT FROM new_val) THEN
        INSERT INTO field_audit_log (table_name, record_id, field_name, old_value, new_value, changed_by, action)
        VALUES ('employees', NEW.id, field_name, old_val::TEXT, new_val::TEXT, current_user_id, 'UPDATE');
      END IF;
    END LOOP;
    RETURN NEW;
  END IF;
...
DROP TRIGGER IF EXISTS trigger_log_employee_field_changes ON employees;
CREATE TRIGGER trigger_log_employee_field_changes
  AFTER INSERT OR UPDATE ON employees
  FOR EACH ROW
  EXECUTE FUNCTION log_employee_field_changes();

COMMENT ON FUNCTION log_employee_field_changes IS 'Logs individual field changes for employees table, handles null user values gracefully';
