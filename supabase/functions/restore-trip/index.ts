...
Deno.serve(async (req: Request) => {
  ...
  const { passenger_id, session_type, trip_date } = body
  ...
  // --- 6. Time gate: check if restore window has passed ---
  ...
  if (Date.now() >= cutoff.getTime()) {
    ...
    return json({ error: `Restore window has passed. Trip starts at ${hhmm}` }, 422)
  }

  // --- 7. Look up existing route_session ---
  ...
  // --- 8. Find the active absence report to restore ---
  // Match by passenger + route_session (or null session if driver hasn't started yet).
  // status must be 'new' or 'acknowledged' — 'resolved' or 'cancelled' rows are already closed.
  const absenceQuery = serviceClient
  ...
  const { data: absenceRows } = tripId !== null
    ? await absenceQuery.eq('route_session_id', tripId)
    : await absenceQuery.is('route_session_id', null)

  const absenceReport = absenceRows?.[0] ?? null

  if (!absenceReport) {
    return json({ error: 'No active cancellation found for this trip' }, 404)
  }

  // --- 9. UPDATE absence report status to 'cancelled' (voided by parent) ---
  const { error: updateError } = await serviceClient
    .from('parent_absence_reports')
    .update({ status: 'cancelled' })
    .eq('id', absenceReport.id)

  if (updateError) {
    console.error('Error updating parent_absence_reports:', updateError)
    return json({ error: 'Failed to restore trip' }, 500)
  }

  // --- 10. INSERT trip_restored notification ---
  const { error: insertNotifError } = await serviceClient
    .from('notifications')
    .insert({
      notification_type: 'trip_restored',
      status: 'pending',
  ...
  if (insertNotifError) {
    // Non-fatal: trip restored, notification failed — log but don't fail the request
    console.error('Error inserting trip_restored notification:', insertNotifError)
  }

  // --- 11. Return success ---
  return json({ absence_report_id: absenceReport.id }, 200)
})
