-- Remove DBS expiry date requirement from passenger assistants
-- PAs now only require: TAS Badge expiry date (DBS expiry removed, DBS number added)
...
-- ======================
-- Update PA Trigger to check for NULL dates (TAS Badge only)
-- ======================
...
-- Summary Comments
-- ======================
COMMENT ON FUNCTION update_expiry_flags() IS 'Updates can_work flag for employees. Drivers require: TAS Badge expiry date only. PAs require: TAS Badge expiry date only. DBS expiry date is no longer required for PAs.';
COMMENT ON FUNCTION trigger_update_pa_expiry() IS 'Automatically flags PAs as cannot work if required certificate (TAS Badge expiry date) is NULL or expired. DBS expiry date is no longer required.';
