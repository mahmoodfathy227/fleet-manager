-- ====================================================
-- Require Certificate Dates for Drivers & PAs
-- Employees cannot work if required certificates are NULL or expired
-- ====================================================

-- Update the expiry flags function to check for NULL dates
...
BEGIN
  ...
  -- ⚠️ NEW: Flag if REQUIRED certificates are NULL or expired
  -- ======================
  ...
  AND id IN (
    ...
    LEFT JOIN passenger_assistants pa ON pa.employee_id = e.id
    WHERE (
      -- Not a driver, OR driver has all required certs set and valid
      (d.employee_id IS NULL OR (
        d.tas_badge_expiry_date IS NOT NULL AND
        d.taxi_badge_expiry_date IS NOT NULL AND
        d.dbs_expiry_date IS NOT NULL AND
        d.tas_badge_expiry_date >= CURRENT_DATE AND
        d.taxi_badge_expiry_date >= CURRENT_DATE AND
        d.dbs_expiry_date >= CURRENT_DATE
      ))
      AND
  ...
  RAISE NOTICE 'Expiry flags updated successfully (NULL checks enabled)';
END;
$$ LANGUAGE plpgsql;

-- ======================
-- Update Driver Trigger to check for NULL dates
-- ======================
...
-- Drop and recreate driver triggers
...
-- ======================
-- Update PA Trigger to check for NULL dates
-- ======================
...
-- ======================
-- Run the function immediately to flag existing records
-- ======================
...
-- Summary Comments
-- ======================
COMMENT ON FUNCTION update_expiry_flags() IS 'Updates can_work flag for employees. Drivers require: TAS Badge, Taxi Badge, DBS (all must be set and not expired). PAs require: TAS Badge, DBS (all must be set and not expired).';
COMMENT ON FUNCTION trigger_update_driver_expiry() IS 'Automatically flags drivers as cannot work if required certificates (TAS Badge, Taxi Badge, DBS) are NULL or expired';
COMMENT ON FUNCTION trigger_update_pa_expiry() IS 'Automatically flags PAs as cannot work if required certificates (TAS Badge, DBS) are NULL or expired';
