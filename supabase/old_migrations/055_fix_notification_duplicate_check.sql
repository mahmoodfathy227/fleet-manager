  -- ====================================================
  -- Fix notification duplicate check
  -- ====================================================
  -- The issue: Notifications are only created if no 'pending' notification exists
  -- But if a notification was already 'sent', it won't create a new one even if the certificate is still expiring
  -- This fix: Check for any active notification (pending or sent) within the threshold period
  -- ====================================================
  ...
  BEGIN
    ...
    FOR v_record IN (
      ...
      -- FIXED: Check for pending OR sent status (not just pending)
    ...
    FOR v_record IN (
      ...
      WHERE NOT EXISTS (
        ...
        WHERE n.entity_type = 'driver'
          ...
          AND n.certificate_type = v_record.cert_type
          AND n.status IN ('pending', 'sent')  -- FIXED: Check for active notifications
          AND n.expiry_date = v_record.expiry_date  -- Same expiry date
      );
    END LOOP;
