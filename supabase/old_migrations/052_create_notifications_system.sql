...
-- ====================================================
-- Tracks compliance certificate expiry notifications for admins
-- ====================================================
...
CREATE TABLE IF NOT EXISTS notifications (
  ...
  notification_type VARCHAR NOT NULL, -- 'vehicle_certificate', 'driver_certificate', 'assistant_certificate'
  entity_type VARCHAR NOT NULL, -- 'vehicle', 'driver', 'assistant'
  entity_id INTEGER NOT NULL, -- ID of vehicle, driver employee_id, or assistant employee_id
  certificate_type VARCHAR NOT NULL, -- e.g., 'tas_badge_expiry_date', 'mot_date', 'insurance_expiry_date'
  certificate_name VARCHAR NOT NULL, -- Human-readable name, e.g., 'TAS Badge', 'MOT', 'Insurance'
  expiry_date DATE NOT NULL,
  days_until_expiry INTEGER NOT NULL, -- Negative if expired, positive if expiring soon
  recipient_employee_id INTEGER REFERENCES employees(id) ON DELETE SET NULL, -- For vehicles: assigned employee, for drivers/assistants: the employee themselves
  recipient_email VARCHAR, -- Email address to send notification to
  status VARCHAR DEFAULT 'pending', -- 'pending', 'sent', 'resolved', 'dismissed'
  email_sent_at TIMESTAMP,
  email_token VARCHAR UNIQUE, -- Unique token for document upload link
...
CREATE INDEX IF NOT EXISTS idx_notifications_email_token ON notifications(email_token);
CREATE INDEX IF NOT EXISTS idx_notifications_created_at ON notifications(created_at);

-- Enable Row Level Security
ALTER TABLE notifications ENABLE ROW LEVEL SECURITY;

-- RLS Policies - Only admins can see all notifications, employees can see their own
...
-- Add comments
COMMENT ON TABLE notifications IS 'Tracks compliance certificate expiry notifications for admins';
COMMENT ON COLUMN notifications.email_token IS 'Unique token for secure document upload link sent via email';

-- ====================================================
-- FUNCTION: Create notifications for expiring certificates
-- ====================================================
-- SECURITY DEFINER allows the function to run with the permissions of the function creator
-- This bypasses RLS policies, which is necessary for system functions
...
BEGIN
  ...
  FOR v_record IN (
    ...
    -- Use gen_random_bytes if pgcrypto is available, otherwise use random() with timestamp
    BEGIN
...
COMMENT ON FUNCTION create_certificate_notifications() IS 'Creates notifications for expiring compliance certificates (vehicles, drivers, passenger assistants). Should be run daily via cron.';
